...
const SendButton = (prop: SendButtonProp) => {
  const {status, txid, isLoading, disabled, checkDone, symbol, explorerUrl} = prop;

  const {text, color} = toStatusBadge(status)

  return (
    <VStack p={4}>
      <Button mt={4} colorScheme='blue' size='lg' type='submit' isLoading={isLoading} disabled={disabled}>
      ...
      {txid && (
        <HStack>
          {status < 4 ? (
            <Spinner size='xs' />
          ):status == 4 ? (<CheckCircleIcon color='green.300' w={5} h={5} />):(<WarningTwoIcon color='red.300' w={5} h={5} />)}
